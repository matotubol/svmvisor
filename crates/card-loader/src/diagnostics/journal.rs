...
use svmvisor_card_abi::journal::{self, CommitError};
use uefi_raw::Status;
...
pub fn commit(io: &mut impl JournalIo, record: [u32; 8]) -> Result<(), Status> {
    ...
    impl<I: JournalIo> journal::JournalIo for Adapter<'_, I> {
        type Error = Status;
        fn read(&mut self, offset: u64) -> Result<u32, Status> {
            self.0.read(offset)
        }
    ...
    journal::commit_record(&mut Adapter(io), record).map_err(|error| match error {
