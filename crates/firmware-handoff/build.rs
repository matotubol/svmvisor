fn main() {
    ...
    println!(
        "cargo:rustc-env=SVMVISOR_SELECTED_UEFI_SMP={}",
        if smp == "1" { "1" } else { "0" }
    );
    ...
    let page = if raw_page.is_empty() {
        0
    } else if let Some(hex) = raw_page
        .strip_prefix("0x")
        .or_else(|| raw_page.strip_prefix("0X"))
    {
        u64::from_str_radix(hex, 16).expect("SVMVISOR_SIPI_PAGE must be a hexadecimal address")
    } else {
        raw_page
            .parse::<u64>()
            .expect("SVMVISOR_SIPI_PAGE must be a decimal address")
    };
    assert!(
        raw_page.is_empty() || smp == "1",
        "SVMVISOR_SIPI_PAGE requires SVMVISOR_UEFI_SMP=1"
    );
    ...
    let value = if raw.is_empty() {
        ...
        raw.parse::<u64>()
            .expect("SVMVISOR_ARENA_BASE must be a decimal address")
    };
    if !raw.is_empty() {
