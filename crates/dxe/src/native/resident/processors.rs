...
fn admission_error(predicate:u32,processor:usize,item:u64,observed:u64,expected:u64,status:Status)->Error {
    let mut f=AdmissionFailure::new(2,predicate,item,observed,expected,status.0 as u64);
    f.processor=u32::try_from(processor).unwrap_or(u32::MAX);Error::Admission(f)
}
fn firmware(status:Status,predicate:u32,processor:usize)->Result<(),Error>{
    if status==Status::SUCCESS{Ok(())}else{Err(admission_error(predicate,processor,0,0,0,status))}
}
...
pub fn capture_identity() -> Result<Identity, Error> {
    ...
    if vendor != *b"AuthenticAMD" || basic.eax < 1 || extended.eax < 0x80000008 {
        let(item,observed,expected):(u64,u32,u32)=if vendor!=*b"AuthenticAMD" {
            if basic.ebx!=0x68747541{(0,basic.ebx,0x68747541)}else if basic.edx!=0x69746e65{(1,basic.edx,0x69746e65)}else{(2,basic.ecx,0x444d4163)}
        }else if basic.eax<1{(3,basic.eax,1)}else{(4,extended.eax,0x80000008)};
        return Err(admission_error(19,usize::MAX,item,observed as u64,expected as u64,Status::UNSUPPORTED));
    }
    ...
    if !(32..=52).contains(&width) {
        return Err(admission_error(20,usize::MAX,0x80000008,width as u64,32|52<<32,Status::UNSUPPORTED));
    }
    let topology = ext.ecx & (1 << 22) != 0;
    if topology && extended.eax < 0x8000001e {
        return Err(admission_error(21,usize::MAX,0x80000000,extended.eax as u64,0x8000001e,Status::UNSUPPORTED));
    }
    let apic_id = if topology {
        __cpuid_count(0x8000001e, 0).eax
    } else {
        one.ebx >> 24
    };
...
extern "efiapi" fn capture_ap(argument: *mut c_void) {
    ...
    let result = if status != Status::SUCCESS {
        ...
        Err(admission_error(12,record.expected,0,who as u64,record.expected as u64,Status::UNSUPPORTED))
    } else {
        (record.read)()
    };
    unsafe {
...
pub unsafe fn inspect_with(
    ...
    if raw.is_null() || raw as usize % align_of::<MpServicesProtocol>() != 0 {
        return Err(admission_error(2,usize::MAX,raw as u64,raw as u64,align_of::<MpServicesProtocol>() as u64,Status::UNSUPPORTED));
    }
...
unsafe fn inspect_protocol(
    ...
    unsafe {
        firmware(((*mp).get_number_of_processors)(
            mp,
            &mut total,
            &mut enabled,
        ),3,usize::MAX)?;
    ...
    if total == 0 || total > MAX_PROCESSORS || enabled != total || bsp >= total {
        let(item,observed,expected)=if total==0{(0,total,1)}else if total>MAX_PROCESSORS{(1,total,MAX_PROCESSORS)}
            else if enabled!=total{(2,enabled,total)}else{(3,bsp,total)};
        return Err(admission_error(5,bsp,item,observed as u64,expected as u64,Status::UNSUPPORTED));
    }
    ...
    for number in 0..total {
        ...
        {
            return Err(admission_error(if info.status_flag!=expected_flags{7}else{8},number,
                info.processor_id,info.status_flag as u64,expected_flags as u64,Status::UNSUPPORTED));
        }
        inventory.processors[number] = Processor {
    ...
    for number in 0..total {
        let identity = if number == bsp {
            read().map_err(|error|match error{
                Error::Admission(mut f)=>{f.processor=number as u32;Error::Admission(f)}})?
        } else {
            let record = Capture {
                mp,
                expected: number,
                read,
                result: UnsafeCell::new(Err(admission_error(10,number,0,0,2,Status::NOT_READY))),
            ...
            if completed != 2 {
                return Err(admission_error(10,number,0,completed as u64,2,Status::UNSUPPORTED));
            }
        ...
        if inventory.processors[number].information.processor_id != u64::from(identity.apic_id) {
            let mut f=admission_error(13,number,0,identity.apic_id as u64,
                inventory.processors[number].information.processor_id,Status::UNSUPPORTED);
            let Error::Admission(ref mut value)=f;value.apic_id=identity.apic_id;
            return Err(f);
        }
    ...
    if (total, enabled, bsp) != (final_total, final_enabled, final_bsp) {
        let (item,observed,expected)=if total!=final_total{(0,final_total,total)}else if enabled!=final_enabled{(1,final_enabled,enabled)}else{(2,final_bsp,bsp)};
        return Err(admission_error(16,final_bsp,item,observed as u64,expected as u64,Status::UNSUPPORTED));
    }
    for processor in inventory.processors() {
        let mut info = ProcessorInformation::default();
        unsafe {
            firmware(((*mp).get_processor_info)(
                mp,
                processor.firmware_number,
                &mut info,
            ),17,processor.firmware_number)?;
        }
        if info != processor.information {
            let (item,observed,expected)=if info.processor_id!=processor.information.processor_id {
                (0,info.processor_id,processor.information.processor_id)
            }else if info.status_flag!=processor.information.status_flag{(1,info.status_flag as u64,processor.information.status_flag as u64)}
            else if info.package!=processor.information.package{(2,info.package as u64,processor.information.package as u64)}
            else if info.core!=processor.information.core{(3,info.core as u64,processor.information.core as u64)}
            else{(4,info.thread as u64,processor.information.thread as u64)};
            return Err(admission_error(18,processor.firmware_number,item,observed,expected,Status::UNSUPPORTED));
        }
    }
    Ok(inventory)
}
...
mod tests {
    ...
    unsafe extern "efiapi" fn who(_: *const MpServicesProtocol, number: *mut usize) -> Status {
        if MODE.load(Ordering::Relaxed)==14&&CURRENT.load(Ordering::Relaxed)==1{return Status::DEVICE_ERROR;}
        unsafe {
            number.write(
    ...
    unsafe extern "efiapi" fn startup(
        ...
        if mode==15&&number==1{
            let record=unsafe{&*argument.cast::<Capture>()};
            assert!(matches!(unsafe{*record.result.get()},Err(Error::Admission(f)) if f.operation==4&&f.observed==0xfedcba9876543210));
            return Status::TIMEOUT;
        }
        Status::SUCCESS
    }
    ...
    fn identity() -> Result<Identity, Error> {
        let mode=MODE.load(Ordering::Relaxed);let current=CURRENT.load(Ordering::Relaxed);
        if ((mode==13||mode==15)&&current==1)||(mode==16&&current==2){
            let mut f=AdmissionFailure::new(4,8,0xc0010010,0xfedcba9876543210,0x123456789abcdef0,48);
            f.apic_id=(current as u32+1)*17;return Err(Error::Admission(f));
        }
        Ok(Identity {
    ...
    fn returned_inventory_binds_sparse_ids_nonzero_bsp_and_refuses_incomplete_callbacks() {
        ...
        for (mode, predicate) in [
            ...
            if mode==1||mode==15{assert_eq!(failure.status,Status::TIMEOUT.0 as u64);assert_eq!(failure.operation,2);}
            if mode==14{assert_eq!(failure.status,Status::DEVICE_ERROR.0 as u64);assert_eq!(failure.processor,1);}
            if mode==13||mode==16{
                assert_eq!(failure.operation,4);assert_eq!(failure.processor,if mode==13{1}else{2});
                assert_eq!(failure.apic_id,if mode==13{34}else{51});
                assert_eq!(failure.observed,0xfedcba9876543210);assert_eq!(failure.expected,0x123456789abcdef0);
                let contexts=failure.contexts(3);assert_eq!(contexts[2],failure.observed);assert_eq!(contexts[3],failure.expected);
            }
