...
pub(super) fn terminal_endpoint() -> *const TerminalEndpoint {
    unsafe { match &*ptr::addr_of!(TERMINAL) {
...
pub(super) fn protect_config(npt: &mut svmvisor_hypervisor::memory::npt::IdentityNpt<'_>)
    -> Result<(), svmvisor_hypervisor::memory::npt::IdentityNptError>
{
    if let Some(endpoint) = unsafe { *ptr::addr_of!(TERMINAL) } {
        let (base,bytes)=endpoint.config_aperture().ok_or(
            svmvisor_hypervisor::memory::npt::IdentityNptError::InvalidExclusion)?;
...
unsafe fn terminal_config_matches(endpoint: TerminalEndpoint) -> bool {
    ...
    read(0) == terminal::PCI_VENDOR_DEVICE && read(8) == terminal::PCI_CLASS_REVISION
        && ((read(0x0c) >> 16) & 0xff) == 0
        && read(4) as u16 == endpoint.command && read(0x10) == endpoint.bar0_raw
}
...
static mut ADMISSION_FAILURE:Option<(svmvisor_dxe::diagnostics::resident_boot::AdmissionFailure,u32)>=None;
...
pub(super) fn admission_failure(value:svmvisor_dxe::diagnostics::resident_boot::AdmissionFailure,count:u32){
    unsafe{ADMISSION_FAILURE=Some((value,count));}
    preparation_failure(32,Status::UNSUPPORTED.0 as u64,
...
unsafe fn publish_admission_failure_with(processor:Option<Cpu>,map:&[MemoryDescriptor]){
    ...
    let result=(||->Result<(),Status>{
        ...
        if !unsafe{terminal_config_matches(endpoint)}{return Err(Status::DEVICE_ERROR);}
        let mut io=Direct(endpoint.bar0_host_page);
        if io.read(0)?!=0x4a4d5653||io.read(4)?!=0x00030001
            ||(io.read(8)? as u64|((io.read(12)? as u64)<<32))!=endpoint.fpga_build_id
            ||(io.read(16)? as u64|((io.read(20)? as u64)<<32))!=endpoint.rom_build_id
            ||io.read(0x84)?!=endpoint.boot_id{return Err(Status::DEVICE_ERROR);}
        let low:u32;let high:u32;
        unsafe{asm!("rdtsc",out("eax")low,out("edx")high,options(nostack,preserves_flags));}
        let payload=terminal::diagnostic_payload(1,12,true,endpoint.boot_id,value.apic_id,
            low as u64|((high as u64)<<32),value.contexts(count),value.operation);
        ...
        let bank=if value.processor<count{value.processor as usize}else{unsafe{physical::bsp_slot()}};
        terminal::commit_diagnostic(&mut io,bank,payload).map_err(|_|Status::DEVICE_ERROR)
    })();
    if result.is_err(){JOURNAL_LOST.store(true,Ordering::Release);}
}
...
pub(super) unsafe fn activation_failure(result: u64) {
    if result == 48 {
        ...
        unsafe { publish_admission_failure(); }
    }
    if result == 35 {
...
pub(super) fn preparation_step(stage: u32, address: u64) {
    unsafe { PREPARATION = (stage, 0, 0, address); }
}
pub(super) fn preparation_failure(reason: u32, status: u64, address: u64) {
    unsafe { PREPARATION = (PREPARATION.0, reason, status, address); }
}
...
impl Prepared {
    ...
    pub(super) unsafe fn new(image: Handle, bs: &BootServices) -> Result<Option<Self>, Status> {
        ...
        let checked = (|| {
            ...
            if io.read(0)? != 0x4a4d5653 || io.read(4)? & !0x00020000 != 0x00010001 || io.read(0x24)? != 0 {
            ...
            if let Some(endpoint) = value.terminal_endpoint() {
                ...
                if unsafe { validate_uc_mmio(map.descriptors(), cfg, &mt, pat, endpoint.config_page) }.is_ok()
                    && unsafe { terminal_config_matches(endpoint) }
                    && (u64::from(io.read(8)?) | (u64::from(io.read(12)?) << 32)) == endpoint.fpga_build_id
                    && (u64::from(io.read(16)?) | (u64::from(io.read(20)?) << 32)) == endpoint.rom_build_id
                {
                    unsafe { TERMINAL = Some(endpoint); }
                }
            }
            Ok(Self(options))
        })();
    ...
    pub(super) fn complete(self, result: Status, armed: bool) {
        if !armed {unsafe{publish_admission_failure();}}
        ...
        unsafe {
            if !armed && (*self.0).version >= 2 {
                ...
                ptr::addr_of_mut!((*self.0).preparation_status)
                    .write_volatile(if reason == 0 { result.0 as u64 } else { status });
...
pub(super) unsafe fn stage(stage: u32, slot: u32, detail: u32) {
    ...
    let detail = if stage == 5 {
        0x0100_0000 | u32::from(!terminal_endpoint().is_null())
    } else {
        detail
    };
...
impl JournalIo for Direct {
    ...
    fn write(&mut self, offset: u64, value: u32) -> Result<(), Status> {
        if (!(0x40..=0x60).contains(&offset) && !(0x600..=0xffc).contains(&offset)) || offset & 3 != 0 {
            return Err(Status::INVALID_PARAMETER);
        }
        unsafe {
...
impl terminal::JournalIo for Direct {
    type Error=Status;
    fn read(&mut self,offset:u64)->Result<u32,Status>{JournalIo::read(self,offset)}
    fn write(&mut self,offset:u64,value:u32)->Result<(),Status>{JournalIo::write(self,offset,value)}
}
