...
| `fixtures/` | Emulator-only entry paths, negative cases, and transition fixtures. |
...
| `card-load-only` | Validates and prepares a payload without executing the emulator handoff. Requires `SVMVISOR_CARD_PAYLOAD_SHA256` for UEFI builds. |
| `native-preflight` | Separate native child captures entry state and observes admission evidence. |
| `native-resident` / `native-resident-test` | Raw resident payload and single-CPU ReadyToBoot activation; build with `tools/native-resident/build.py`. Test profile adds diagnostic output. |
| `native-resident-smp-prepare` | Diagnostic inventory and per-CPU pool preparation only; returns without registering activation or enabling SVM. |
| `native-resident-smp-activate` | Diagnostic configuration-table entry called after successful EBS return; serial physical INIT/SIPI and actual per-CPU resident continuation, fixed x2APIC mode. |
| `native-resident-guest-startup` | Opt-in post-EBS diagnostic with INIT/#SX notification and native IRQ delivery; actual guest reset requires quiescent LAPIC. Build with `--guest-startup`; see [wakeup scope](../../docs/native-init-sx-wakeup.md). |
| `native-resident-boot` | Native loader EBS interposer, broadcast physical bootstrap and guarded guest startup; build with `--boot`. Optional card options provide BSP-only pre-Windows journal evidence. |
...
| `emulator-handoff` and its variants | Synthetic payload and PCI/MMIO fixtures; use the harness to supply their required inputs. |
...
interposer. It does not imply diagnostic output; the builder adds that separately.
Use `python tools/native-resident/build.py --output work/NEW --boot` to build/audit
without executing firmware. Add `--test-output` only for disposable emulator runs.
...
The native xAPIC path does not require advertised x2APIC support. Mode selection
and actual promotion retain capability checks; see
`docs/native-bootstrap-apic-admission.md` for the physical refusal and correction.
The earlier `--guest-startup`, single ReadyToBoot and explicit SMP activation
profiles remain regression fixtures. See [loader handoff](../../docs/native-loader-handoff.md)
