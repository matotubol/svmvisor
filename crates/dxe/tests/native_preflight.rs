...
fn amd(leaf: u32) -> R {
    match leaf {
        ...
        0x80000002..=0x80000004 => {
            ...
            let word = |index| {
                u32::from_le_bytes(
                    bytes[offset + index..offset + index + 4]
                        .try_into()
                        .unwrap(),
                )
            };
...
fn bounded_collection_reaches_explicit_boundary_refusal() {
    ...
    assert_eq!(
        calls,
        [
            0, 0x80000000, 1, 0x80000001, 0x80000008, 0x8000000a, 0x80000002, 0x80000003,
            0x80000004
        ]
    );
...
fn reported_hypervisor_reaches_core_refusal_unchanged() {
    ...
    assert_eq!(
        report.outcome,
        Outcome::CpuidRejected(PreflightError::HypervisorReported)
    );
    assert_eq!(report.outcome.diagnostic_code(), 3);
    assert_ne!(report.evidence.features.unwrap().ecx & (1 << 31), 0);
    assert!(
        report.identity.is_ok(),
        "identity does not override SVM refusal"
    );
}
...
fn complete_brand_group_is_collected_only_when_all_three_leaves_are_enumerated() {
    for maximum in 0x80000000..=0x8000000a {
        ...
        let report = collect(|leaf| {
            ...
            assert!(
                leaf <= 1 || leaf <= maximum,
                "unsupported extended leaf {leaf:x}"
            );
            if leaf == 0x80000000 {
                R {
                    eax: maximum,
        ...
        let brand_calls: Vec<_> = calls
            .iter()
            .copied()
            .filter(|leaf| (0x80000002..=0x80000004).contains(leaf))
            .collect();
...
fn invalid_identity_is_reported_without_changing_existing_preflight_outcome() {
    for (leaf, altered, identity_error) in [
        (
            1,
            R { eax: 0, ..amd(1) },
            CpuIdentityError::InvalidSignature,
        ),
        (
            0x80000001,
            R {
        ...
        (
            ...
            CpuIdentityError::InconsistentVendor,
        ),
    ] {
...
fn missing_basic_signature_and_invalid_extended_maximum_never_create_identity() {
    ...
    assert!(
        calls
            .iter()
            .all(|leaf| !(0x80000002..=0x80000004).contains(leaf))
    );
    assert_eq!(report.identity, Err(CpuIdentityError::InvalidMaxima));
