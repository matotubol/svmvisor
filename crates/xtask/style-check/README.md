...
|---|---|
| `bash crates/xtask/style-check/verify.sh quick` | `cargo fmt --check` for all crates, then all 6 test configurations. About two minutes warm. |
...
| `python crates/xtask/style-check/sdiff.py BASE.s NEW.s` | The same layout-insensitive per-function comparison for two rustc `--emit=asm` files, for `card-loader` and `launcher` images. `--dump NAME` prints one normalized body. |
...
- The disassembly comparison covers the **resident payload** (the hypervisor
  ...
  sets with `--emit=asm` before and after and compare them with `sdiff.py`.
  Never use `asmdiff.py` on `.s` files: it finds no functions and reports no
  differences. A pinned loader needs a structurally valid
  `SVMVISOR_CARD_PE_HEADER`; with an all-zero pin LTO folds the delivery path away.
