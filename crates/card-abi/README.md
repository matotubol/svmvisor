...
| `package` | The `SVMRELO1` relocatable package an `SVMCRD01` envelope wraps: `ARENA_BYTES`, `HANDOFF_OFFSET`, `Payload::{parse, load}`, `LayoutError`, `is_valid_arena`. Pure `core`; shared by the loader's load-only image, the native resident launcher and the emulator handoff. |
