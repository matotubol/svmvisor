...
| `envelope` | The 128-byte header of the card's 1 MiB payload slot: the `SVMBPE01` magic, field offsets, the flag and the PE policy limits; `Envelope::parse`, `parse_pe`, `PeMetadata` and `EnvelopeError`, the parser the loader and `xtask` share. |
