...
pub fn prepare_io(vmcb: &mut Vmcb) -> Result<PreparedIo<'_>, ConfigError> {
    ...
    if io.string() || io.rep() {
        return Err(ConfigError::StringOrRep);
    }
    ...
    if !overlaps {
        return Err(ConfigError::PortOrWidth);
    }
