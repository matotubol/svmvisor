...
pub fn native_pause_retry_ready(vmcb: &Vmcb) -> bool {
    ...
    vmcb.exit_snapshot().code == 0x77
        && u32::from_le_bytes(bytes[0x00c..0x010].try_into().unwrap()) & (1 << 23) != 0
        && u16::from_le_bytes(bytes[0x03c..0x03e].try_into().unwrap()) == 0
        && u16::from_le_bytes(bytes[0x03e..0x040].try_into().unwrap()) == 4096
}
