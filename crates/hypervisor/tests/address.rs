...
fn inclusive_last_byte_boundary_and_overflow() {
    for bits in [32, 48, 52] {
        ...
        assert_eq!(
            p.validate(limit - 4096, 4097, 4096),
            Err(E::OutsidePhysicalWidth)
        );
...
fn encryption_evidence_and_encoded_ranges_are_rejected() {
    assert_eq!(
        AddressPolicy::new(48, EncryptionState::Unknown),
        Err(E::UnknownEncryption)
    );
    assert_eq!(
    ...
    assert_eq!(
        p.validate(0, 2 * bit + 1, 4096),
        Err(E::EncryptionBitEncoded)
    );
