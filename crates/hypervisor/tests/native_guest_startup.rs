...
fn native_running_init_preserves_retained_state_and_starts_real16_repeatedly() {
    ...
    for vector in [0x08, 0xff, 0] {
        ...
        assert_eq!(*target.frame, GuestRegisters { rdx: 0xa20f12, ..GuestRegisters::default() });
        ...
        assert_eq!(&target.vmcb.bytes()[0x410..0x414], &[0x00, 0xf0, 0x9a, 0x00]);
        ...
        assert_eq!(&target.vmcb.bytes()[0x410..0x412], &(u16::from(vector) << 8).to_le_bytes());
...
fn target_pending_refusal_does_not_erase_cpu_state_or_mailbox_head() {
    ...
    let mut target =
        NativeStartupTarget { vmcb: &mut vmcb, frame: &mut frame, state: &mut state, signature: 1 };
...
fn mailbox_readiness_fifo_capacity_and_completion_head_are_checked() {
    ...
    let mailbox = NativeStartupMailbox::new(19);
    assert_eq!(mailbox.publish(Command::Init), Err(NativeIcrError::MailboxNotReady));
    assert_eq!(mailbox.peek(), None);
    mailbox.mark_running();
    let commands = [Command::Init, Command::Sipi(8), Command::Init, Command::Sipi(9)];
    for command in commands {
        mailbox.publish(command).unwrap();
    }
    assert_eq!(mailbox.publish(Command::Sipi(10)), Err(NativeIcrError::MailboxBusy));
    assert_eq!(mailbox.complete(Command::Sipi(8)), Err(NativeIcrError::MailboxMismatch));
    for command in commands {
...
fn nmi_ipi_queues_the_nmi_command_on_remote_mailboxes_and_kicks() {
    ...
    let mailboxes = [
        NativeStartupMailbox::new(4),
        NativeStartupMailbox::new(9),
...
fn x2avic_cpu_startup_preserves_hardware_bindings_and_rejects_stale_profile() {
    use svmvisor_hypervisor::{
        memory::address::{AddressPolicy, EncryptionState},
        svm::x2avic::{NativeX2AvicProfile, X2AvicCapabilities},
    };
    let policy =
        AddressPolicy::new(48, EncryptionState::Unencrypted { encryption_bit: None }).unwrap();
    let caps = X2AvicCapabilities::admit(1 << 21, 1 | (1 << 13) | (1 << 18) | (1 << 25)).unwrap();
        ...
        thread::scope(|scope| {
            ...
            for _ in 0..2_000_000 {
                if let Some(command) = mailbox.peek() {
                    let Command::Sipi(vector) = command else { panic!("unexpected INIT") };
...
fn x2avic_startup_routes_full_identity_and_notifies_after_publication() {
    let boxes = [
        NativeStartupMailbox::new(7),
        NativeStartupMailbox::new(19),
        NativeStartupMailbox::new(275),
    ];
    for b in &boxes {
        b.mark_running();
    }
    ...
    for (low, expected) in [(0xc500, Command::Init), (0x608, Command::Sipi(8))] {
        owner
            .route_x2avic_startup((275u64 << 32) | low, &boxes, |id| {
                assert_eq!(id, 275);
...
fn x2avic_startup_refusal_and_broadcast_publication_are_atomic() {
    ...
    for _ in 0..4 {
        boxes[2].publish(Command::Sipi(9)).unwrap();
    }
    let broadcast = (0xdead_beefu64 << 32) | (3 << 18) | (1 << 11) | 0x500;
    assert_eq!(
        owner.route_x2avic_startup(broadcast, &boxes, |_| panic!()),
        Err(NativeIcrError::MailboxBusy)
    );
    assert_eq!(boxes[0].peek(), None);
    assert_eq!(boxes[1].peek(), None);
    for _ in 0..4 {
        boxes[2].complete(Command::Sipi(9)).unwrap();
    }
    owner
        .route_x2avic_startup(broadcast, &boxes, |id| {
