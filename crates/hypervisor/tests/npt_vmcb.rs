...
fn planned_root_transfers_to_vmcb_without_enabling_npt_or_fabricating_exit() {
    ...
    .unwrap();
    npt.map_page(0x2000, 0x200000, PagePermissions::ReadExecute)
        .unwrap();
    npt.map_page(0x3000, 0x201000, PagePermissions::ReadWrite)
        .unwrap();
    assert_eq!(
        npt.translate(0x2fff).unwrap().unwrap().host_address,
        0x200fff
    );
    assert_eq!(
        npt.translate(0x3000).unwrap().unwrap().permissions,
    ...
    assert_eq!(
        vmcb.exit_snapshot().action(),
        ExitAction::Unsupported { code: 0 }
    );
