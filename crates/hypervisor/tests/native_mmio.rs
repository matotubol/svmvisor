type NativeMmioError = MmioError<NativeIcrError>;
...
use svmvisor_hypervisor::{
    ...
    svm::{
        ipi::NativeIcrError,
        vmcb::Vmcb,
        native_mmio::{NativeMmioError as MmioError, handle_native_mmio},
    },
};

#[test]
fn device_dword_offset_and_exact_x2avic_bindings_are_checked_before_callback() {
    use svmvisor_hypervisor::{address::{AddressPolicy, EncryptionState},
        svm::{native_mmio::handle_x2avic_mmio_detailed, x2avic::{NativeX2AvicProfile, X2AvicCapabilities}}};
    ...
    let profile = NativeX2AvicProfile::new(caps, 0x2000, 0x3000, 23, &policy).unwrap();
    let wrong = NativeX2AvicProfile::new(caps, 0x5000, 0x3000, 23, &policy).unwrap();
    for valid in [false, true] {
        let mut guest = Guest::new(&[0x89, 3], true);
        guest.frame.rbx = 0x6304; // Device DWORD, not a 16-byte APIC register stride.
        put(&mut guest.vmcb, 0x80, 0xfee00304);
        guest.vmcb.enable_native_x2avic(&profile).unwrap();
        let before = *guest.vmcb.bytes();
        let frame = guest.frame;
        let result = handle_x2avic_mmio_detailed(&mut guest.vmcb, &mut guest.frame, 48,
            guest.pat, 0xfee00000, if valid { &profile } else { &wrong },
            |address, width| if width == 8 {guest.tables.get(&address).copied()}
                else {guest.code.get(&address).map(|b| *b as u64)},
            |_, offset, write| { assert!(valid); assert_eq!((offset, write), (0x304, Some(0x4600))); Ok::<_, ()>(0) });
        if valid {
            result.unwrap();
            assert_eq!(guest.vmcb.guest_rip(), 0x4002);
        } else {
            assert!(matches!(result.unwrap_err().error, MmioError::PendingState(_)));
            assert_eq!(guest.vmcb.bytes(), &before);
        }
        assert_eq!(guest.frame, frame);
    }
}
...
fn detailed_refusals_capture_actual_predicate_operand_without_mutation_or_extra_reads() {
    use svmvisor_hypervisor::svm::native_mmio::handle_native_mmio_detailed;
    for (bytes, change, reason, operand) in [
        ...
        let failure=handle_native_mmio_detailed::<NativeIcrError>(&mut g.vmcb,&mut g.frame,48,g.pat,0xfee00000,
        ...
        assert_eq!((u64::from(failure.predicate), failure.operand), (reason, operand));
...
fn cet_without_wp_is_refused_before_any_guest_or_device_read() {
    ...
    for write in [false, true] {
        ...
        let before = *g.vmcb.bytes();
        let frame = g.frame;
        let failure = handle_native_mmio_detailed::<NativeIcrError>(&mut g.vmcb, &mut g.frame, 48, g.pat,
...
fn detailed_fetch_distinguishes_instruction_and_operand_and_retains_physical_failure_address() {
    ...
    for data in [false,true] {
        ...
        assert_eq!(failure.predicate, if data {0x21} else {0x20});
        assert_eq!(failure.error, NativeMmioError::Fetch(if data {
            FetchError::Walk(WalkError::UnreadableTable { level: 1, address: 0x4030 })
        } else { FetchError::UnreadableInstruction { address: 0x9000 } }));
    }
}
