use svmvisor_hypervisor::{
    ...
    svm::{
        dispatch::{
            NativeEfer, NativeMsrOutcome, handle_native_efer, handle_native_efer_with_nrip,
        },
        vmcb::Vmcb,
    },
};
...
fn decoded_prefix_lengths_complete_without_guest_memory() {
    for length in [2, 3, 4, 7, 15] {
        for write in [false, true] {
            ...
            let rip = v.guest_rip();
            put(&mut v, 0xc8, rip + length);
            put(&mut v, 0x78, write as u64);
            if write {
                put(&mut v, 0x5f8, 0xd01);
                f.rdx = 0;
            }
            let old = f;
            ...
            assert_eq!(
                handle_native_efer_with_nrip(&mut e, &mut v, &mut f, &caps),
...
fn malformed_hardware_evidence_and_pending_state_refuse_transactionally() {
    for case in 0..20 {
        ...
        match case {
            ...
            5 => put(&mut v, 0xc8, 0x0000800000000000),
            ...
            17 => put(&mut v, 0x578, 0x0000800000000000),
            18 => f.rcx = 0x1b,
            _ => put(&mut v, 0x4d0, 0x1d00),
        }
        let before = (e, *v.bytes(), f);
        assert!(
            handle_native_efer_with_nrip(&mut e, &mut v, &mut f, &capabilities(supported)).is_err(),
            "case {case}"
        );
...
fn nrip_boundaries_are_checked_without_wrapping() {
    for (rip, next, ok) in [
        (0x7ffffffffffd, 0x7fffffffffff, true),
        (0x7ffffffffffe, 0x800000000000, false),
        (0xfffffffffffffff0, 0xffffffffffffffff, true),
        (0xfffffffffffffffe, 0, false),
    ] {
        let (mut v, mut f) = stopped(0x2000);
        let mut e = owner();
        put(&mut v, 0x578, rip);
        ...
        let before = (e, *v.bytes(), f);
        assert_eq!(
            handle_native_efer_with_nrip(&mut e, &mut v, &mut f, &capabilities(true)).is_ok(),
            ok
        );
        if ok {
            assert_eq!(v.guest_rip(), next);
        } else {
...
fn prefixed_fault_uses_original_rip_and_real_efer_fault_owner() {
    for length in [3, 15] {
        ...
        assert_eq!(
            handle_native_efer_with_nrip(&mut e, &mut v, &mut f, &capabilities(true)),
...
fn hardware_diagnostics_report_real_nrip_not_synthetic_opcode_length() {
    use svmvisor_hypervisor::{
        host::resident::terminal,
        svm::{dispatch::NativeEferError, exit::ResumeError},
    };
    let (mut v, _) = stopped(0x2000);
    put(&mut v, 0xc8, 0x800000000000);
    let (_, payload) = terminal::efer_nrip_failure(
        NativeEferError::Instruction(ResumeError::NonCanonicalNrip),
        &v,
        0x500,
    );
