use svmvisor_hypervisor::{
    ...
    svm::{
        dispatch::{
            NATIVE_VM_CR_VALUE, NativeMsrOutcome, NativeVmCrError, handle_native_vmcr,
            handle_native_vmcr_with_nrip,
        },
        vmcb::Vmcb,
    },
};
...
fn all_bits_and_mixed_writes_have_transactional_outcomes() {
    for value in (0..64).map(|bit| 1u64 << bit).chain([0, 8, 16, 24, 0xffffffffffffffff, 0x22]) {
        for hardware in [false, true] {
            ...
            put(&mut v, 0x78, 1);
            put(&mut v, 0x5f8, 0xdeadbeef00000000 | value as u32 as u64);
            f.rdx = 0xabcdef0000000000 | value >> 32;
            let before = (*v.bytes(), f);
            let oldrax = v.guest_rax();
            let result = if hardware {
                handle_native_vmcr_with_nrip(&mut v, &mut f, &capabilities(true), true)
            } else {
                handle_native_vmcr(&mut v, &mut f, &[0x0f, 0x30], true)
            };
            if value & !0x1f != 0 {
                assert_eq!(result, Ok(NativeMsrOutcome::GeneralProtectionPrepared), "{value:x}");
                assert_eq!(v.guest_rip(), 0x2000);
                assert_eq!(word(&v, 0xa8), 0x80000b0d);
                assert_eq!(word(&v, 0x570), 0x10002);
            } else if value & 7 != 0 {
                assert_eq!(result, Err(NativeVmCrError::UnsupportedValue { value }));
            ...
            assert_eq!(v.guest_rax(), oldrax);
            assert_eq!(word(&v, 0x4d0), 0x1500);
...
fn reads_and_ignored_writes_cover_all_hardware_lengths() {
    for length in 2..=15 {
        for value in [0, 8, 16, 24] {
            ...
            put(&mut v, 0x78, 0);
            put(&mut v, 0xc8, 0x2000 + length * 2);
            f.rdx = u64::MAX;
            let old = f;
            assert_eq!(
                handle_native_vmcr_with_nrip(&mut v, &mut f, &capabilities(true), true),
            ...
            assert_eq!(v.guest_rax(), NATIVE_VM_CR_VALUE);
            assert_eq!(f.rdx, 0);
            assert_eq!(v.guest_rip(), 0x2000 + length * 2);
...
fn malformed_hardware_and_pending_debug_state_preserve_every_byte() {
    for case in 0..19 {
        ...
        let before = (*v.bytes(), f);
        assert!(
            handle_native_vmcr_with_nrip(&mut v, &mut f, &capabilities(supported), true).is_err(),
            "case{case}"
        );
...
fn byte_fallback_faults_cpl_and_refuses_prefixes() {
    ...
    let before = (*v.bytes(), f);
    assert!(handle_native_vmcr(&mut v, &mut f, &[0x48, 0x0f, 0x32], true).is_err());
    assert_eq!((*v.bytes(), f), before);
    put(&mut v, 0x4c8, 3 << 24);
    let before = (v.guest_rip(), v.guest_rax(), f);
    assert_eq!(
        handle_native_vmcr(&mut v, &mut f, &[0x0f, 0x32], true),
