...
fn snapshot(pages: &GuestPages<'_>) -> Vec<Vec<u8>> {
    (0..4)
        .map(|i| pages.table(i).unwrap().bytes.to_vec())
        .collect()
}
...
fn exported_four_level_walk_has_supervisor_permissions_and_no_nx() {
    ...
    for (va, gpa, permissions) in [
        (0x1fff, 0x1fff, P::ReadOnly),
        (0x8fff, 0x8fff, P::ReadWrite),
    ] {
...
fn complete_window_capacity_and_highest_numeric_gpa_are_bounded() {
    ...
    for index in 0..512 {
        pages
            .map_page(index * 4096, (1 << 52) - (512 - index) * 4096, P::ReadWrite)
            .unwrap();
    }
    assert_eq!(
        pages
            .translate(WINDOW_BYTES - 1)
            .unwrap()
            .unwrap()
            .guest_address,
        (1 << 52) - 1
    ...
    let before = snapshot(&pages);
    assert_eq!(
        pages.map_page(WINDOW_BYTES, 0, P::ReadOnly),
        Err(E::VirtualAddressOutsideWindow)
    );
    assert_eq!(snapshot(&pages), before);
...
fn relocated_windows_encode_all_parent_indices_and_preserve_full_virtual_addresses() {
    ...
    for base in [
        ...
        pages
            .map_page(base + (WINDOW_BYTES - 4096), 0x9000, P::ReadWrite)
            .unwrap();
        for (va, gpa, permissions) in [
        ...
        let before = snapshot(&pages);
        for outside in [base.checked_sub(1), base.checked_add(WINDOW_BYTES)]
            .into_iter()
            .flatten()
        {
            assert_eq!(
                pages.translate(outside),
                Err(E::VirtualAddressOutsideWindow)
            );
            assert_eq!(
        ...
        assert_eq!(
            pages.map_page(base + 4096, 0x4000, P::ReadWrite),
            Err(E::GuestPageAlias)
        );
        assert_eq!(
            pages.map_page(base + 4096, 0x101000, P::ReadWrite),
            Err(E::TableArenaOverlap)
        );
        assert_eq!(
...
fn invalid_virtual_window_or_arena_preserves_caller_storage() {
    ...
    for (base, arena, expected) in [
        ...
        (
            0xffff_7fff_ffe0_0000,
            0x100000,
            E::VirtualWindowNonCanonical,
        ),
        (u64::MAX, 0x100000, E::VirtualWindowOverflow),
        (
            u64::MAX - WINDOW_BYTES + 2,
            0x100000,
            E::VirtualWindowOverflow,
        ),
