...
use svmvisor_hypervisor::{
    ...
    guest_pages::{GuestPages, PagePermissions as GuestAccess, TableStorage as GuestStorage},
...
fn guest_code_stacks_descriptors_and_table_walks_have_separate_backing() {
    ...
    for (va, hpa, guest_access, host_access) in [
        (
            0x1000,
            0x200000,
            GuestAccess::ReadOnly,
            HostAccess::ReadExecute,
        ),
        (
            0x4000,
            0x201000,
        ...
        assert_eq!(
            npt.translate(gpa).unwrap().unwrap().host_address,
            hpa + 4095
        );
    }
    ...
    for i in 0..4 {
        ...
        npt.map_page(
            table.guest_address,
            0x210000 + i as u64 * 4096,
            HostAccess::ReadWrite,
        )
        .unwrap();
        assert_eq!(
            npt.translate(table.guest_address)
                .unwrap()
                .unwrap()
                .permissions,
            HostAccess::ReadWrite
    ...
    assert_eq!(
        &vmcb.bytes()[0x410..0x418],
        &[8, 0, 0x9b, 0x0a, 0xff, 0xff, 0xff, 0xff]
    );
    assert_eq!(
        &vmcb.bytes()[0x420..0x428],
        &[16, 0, 0x93, 0x0c, 0xff, 0xff, 0xff, 0xff]
    );
