...
fn seed_cet(v: &mut Vmcb) {
    ...
    for (o, x) in [
        (0x548, 0x20 | (1 << 22) | (1 << 23)),
        (0x5e0, 3),
        (0x5e8, 0xffff800001234000),
        (0x5f0, 0xffff800005678000),
...
fn initial_cet_import_changes_only_sampled_msrs_and_clean_bits() {
    ...
    for s_cet in 0u64..=3 {
        for isst in [0, 0x7fff_ffff_ffff, 0xffff_8000_0000_0000, u64::MAX] {
            let (mut v, _) = fixture();
...
fn initial_cet_import_rejects_reserved_bits_active_cet_and_noncanonical_addresses_unchanged() {
    ...
    for bit in 2..64 {
        check(1u64 << bit, 0, false);
    }
    for isst in [0x8000_0000_0000, 0xffff_7fff_ffff_ffff, 1u64 << 63] {
        check(0, isst, false);
    }
    check(0, 0, true);
}
...
fn init_retains_cet_msrs_and_duplicate_sipi_does_not_erase_shadow_state() {
    use svmvisor_hypervisor::svm::x2avic::startup::{
        NativeStartupCommand as C, NativeStartupEffect as E, NativeStartupState as S,
        NativeStartupTarget,
    };
    ...
    let (mut v, mut f) = fixture();
    seed_cet(&mut v);
    let profile = x2avic(&mut v);
...
fn rejected_init_keeps_every_cet_field_and_frame_unchanged() {
    use svmvisor_hypervisor::svm::x2avic::startup::{
        NativeStartupCommand as C, NativeStartupState as S, NativeStartupTarget,
    };
    let (mut v, mut f) = fixture();
    seed_cet(&mut v);
    let profile = x2avic(&mut v);
    ...
    let mut state = S::Running;
    let mut target =
        NativeStartupTarget { vmcb: &mut v, frame: &mut f, state: &mut state, signature: 0xb40f40 };
    assert!(target.apply_x2avic(C::Init, &profile).is_err());
    assert_eq!(*target.vmcb.bytes(), before);
    assert_eq!(*target.frame, frame);
    assert_eq!(*target.state, S::Running);
}
