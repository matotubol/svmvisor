...
fn gp_zero_encoding_changes_only_injection_and_clean_bits_preserving_fault_rip() {
    for store in [false, true] {
        ...
        let old = *v.bytes();
        v.queue_msr_general_protection(if store { &[0x0f, 0x30] } else { &[0x0f, 0x32] }).unwrap();
...
fn wrong_exit_direction_bytes_and_noncanonical_fault_rip_refuse_transactionally() {
    for (offset, value) in
        [(0x70, u64::MAX), (0x70, 0x81), (0x78, 2), (0x578, 0x0000_8000_0000_0000)]
    {
...
fn pending_and_unsupported_state_cannot_be_overwritten() {
    for (offset, value) in
        [(0xa8, 1 << 31), (0x88, 1 << 31), (0x60, 1 << 8), (0x60, 1 << 31), (0x90, 3)]
    {
