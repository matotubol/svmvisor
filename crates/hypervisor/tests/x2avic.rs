...
fn table_format_and_publication_preserve_identity_and_reserved_bits() {
    ...
    for (id, backing) in [(37, 0xa000), (37, 0x9001), (37, 0x9000 | (1 << 52)), (38, 0x9000), (512, 0x9000)] {
...
fn exit_decode_distinguishes_partial_ipi_and_post_write_eoi() {
    ...
    assert_eq!(AvicExit::decode(0x402, 1, 0), Ok(AvicExit::NoAcceleration { offset: 0, write: false, eoi_vector: None }));
}
...
fn exit_decode_follows_tables_15_25_to_15_29_and_ignores_reserved_fields() {
    ...
    for reason in 0..=4u64 {
        ...
        assert_eq!(AvicExit::decode(0x401, icr, (reason << 32) | 0xffff_fabc),
            Ok(AvicExit::IncompleteIpi { icr, reason: reason as u32, index }), "id {reason}");
    }
    ...
    assert_eq!(AvicExit::decode(0x401, u64::MAX, 0),
    ...
    for (info1, offset, write) in [(0x320 | noise, 0x320, false), ((1 << 32) | 0x320 | noise, 0x320, true),
        ((1 << 32) | 0x3e0, 0x3e0, true), (0xff0, 0xff0, false)] {
        assert_eq!(AvicExit::decode(0x402, info1, u64::MAX),
            Ok(AvicExit::NoAcceleration { offset, write, eoi_vector: None }), "{info1:#x}");
    }
    // Table 15-29: EOI writes carry the highest in-service vector in bits 7:0.
    assert_eq!(AvicExit::decode(0x402, (1 << 32) | 0xb0 | noise, 0xffff_ff00_0000_1f10),
        Ok(AvicExit::NoAcceleration { offset: 0xb0, write: true, eoi_vector: Some(0x10) }));
    assert_eq!(AvicExit::decode(0x402, (1 << 32) | 0xb0, 0xff),
    ...
    assert_eq!(AvicExit::decode(0x402, 0xb0, 0),
...
fn ledger_init_retirement_completes_every_source_and_drains_in_physical_isr_order() {
    use svmvisor_hypervisor::{arch::x86_64::apic::highest_vector, svm::x2avic::irq::{IrqError as E, PhysicalIrqLedger}};
    ...
    assert_eq!(ledger.check_retirement(&missing),
...
fn backing_init_reset_takes_exact_table_16_2_values_and_preserves_identity() {
    ...
    for offset in apic::LVTS { expected.insert(offset, 0x0001_0000); }
    for offset in [apic::TPR, apic::APR, apic::PPR, apic::EOI, apic::RRR, apic::ESR, apic::ICR,
        apic::ICR_HIGH, apic::TIMER_INITIAL_COUNT, apic::TIMER_CURRENT_COUNT, apic::TIMER_DIVIDE] {
        expected.insert(offset, 0);
    }
    for base in [apic::ISR, apic::TMR, apic::IRR] {
        for bank in 0..8 { expected.insert(base + bank * 16, 0); }
    }
...
fn x2avic_cpu_init_commit_zeroes_v_tpr_and_invalidates_clean_bits() {
    ...
    let mut frame = GuestRegisters::default();
    let mut state = NativeStartupState::Running;
    let mut target = NativeStartupTarget { vmcb: &mut vmcb, frame: &mut frame, state: &mut state, signature: 0x00b4_0f40 };
...
fn irq_ledger_holds_level_sources_and_acknowledges_only_in_physical_isr_order() {
    ...
    assert_eq!(ledger.next_eoi(Some(0x40)), Err(E::PhysicalIsrMismatch { vector: 0x80, highest: Some(0x40) }));
...
fn apic_register_numbering_matches_reviewed_backing_offsets() {
    ...
    for (offset, msr) in [(apic::TPR, 0x808), (apic::SVR, 0x80f), (apic::ICR, 0x830),
        (apic::LVT_ERROR, 0x837), (apic::TIMER_INITIAL_COUNT, 0x838), (apic::EXTENDED, 0x840)] {
    ...
    for offset in apic::LVTS { assert_eq!(page.read_register(offset).unwrap(), apic::LVT_MASKED); }
    page.write_register_stopped(apic::TPR, 0x20).unwrap();
