...
fn fixed_wire_layout_round_trips_and_reconstructs_regions() {
    ...
    assert_eq!(restored.arena_end(), original.arena_base() + original.arena_bytes());
    assert_eq!(restored.layout().used_bytes(), original.layout().used_bytes());
    for (left, right) in restored.layout().regions().iter().zip(original.layout().regions()) {
...
fn malformed_headers_are_rejected_without_ignoring_trailing_bytes() {
    ...
    for (offset, error) in
        [(0, HandoffError::Magic), (8, HandoffError::Version), (10, HandoffError::Size)]
    {
        let mut bytes = valid;
        bytes[offset] ^= 1;
        fails(&bytes, error);
...
fn untrusted_arena_metadata_cannot_wrap_or_bypass_layout_validation() {
    ...
    for (offset, value) in
        [(24, 0u64), (24, PAGE_SIZE), (24, 4097), (32, 0), (40, u64::MAX), (48, 0)]
    {
        let mut bytes = valid;
        bytes[offset..offset + 8].copy_from_slice(&value.to_le_bytes());
        assert!(matches!(Handoff::decode(&bytes), Err(HandoffError::Layout(_))));
    }
}
