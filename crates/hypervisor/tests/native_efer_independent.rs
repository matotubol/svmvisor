    ...
    for bit in 0..64 {
        ...
        match bit {
            0 | 8 | 10 | 11 | 14 | 15 | 18 | 20 | 21 => {
                ...
                assert_eq!(o.logical(), value);
                assert_eq!(v.guest_rip(), 0x4002);
                assert_eq!(
                    u64::from_le_bytes(v.bytes()[0x4d0..0x4d8].try_into().unwrap()),
                    value | 0x1000
                );
            }
            1..=7 => {
...
fn same_cpu_capability_absence_faults_each_added_control() {
    for bit in [11, 14, 15, 18, 20, 21] {
        ...
        let mut o =
            NativeEfer::admit_native(0x500, 0, (1 << 11) | (1 << 29), 1 << 20, None).unwrap();
        assert_eq!(
            handle_native_efer(&mut o, &mut v, &mut g, &[15, 48]),
