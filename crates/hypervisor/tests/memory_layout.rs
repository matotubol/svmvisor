...
fn request() -> LayoutRequest {
    LayoutRequest { code_bytes: 1, data_bytes: PAGE_SIZE + 1, stack_bytes: 3 * PAGE_SIZE }
}
...
fn rounded_ranges_are_disjoint_guarded_and_have_explicit_permissions() {
    ...
    let expected = [
        ...
        (RegionKind::ExecutionVmcb, 1, Permissions::ReadWriteNoExecute),
        (RegionKind::HostAuxVmcb, 1, Permissions::ReadWriteNoExecute),
        (RegionKind::NativeReturnVmcb, 1, Permissions::ReadWriteNoExecute),
...
fn exact_capacity_succeeds_and_one_page_short_fails() {
    ...
    assert_eq!(Layout::plan(16 * PAGE_SIZE, request()), Err(LayoutError::InsufficientArena));
...
fn rejects_zero_sizes_and_non_page_arena() {
    for arena in [0, PAGE_SIZE - 1, 17 * PAGE_SIZE + 1] {
        assert_eq!(Layout::plan(arena, request()), Err(LayoutError::InvalidSize));
    }
    for bad in [
        LayoutRequest { code_bytes: 0, ..request() },
        LayoutRequest { data_bytes: 0, ..request() },
        LayoutRequest { stack_bytes: 0, ..request() },
    ] {
        assert_eq!(Layout::plan(17 * PAGE_SIZE, bad), Err(LayoutError::InvalidSize));
    }
}
...
fn rejects_rounding_and_cumulative_overflow() {
    ...
    for bad in [
        LayoutRequest { code_bytes: u64::MAX, ..request() },
        LayoutRequest { data_bytes: u64::MAX, ..request() },
        LayoutRequest { stack_bytes: u64::MAX, ..request() },
        LayoutRequest { code_bytes: arena, ..request() },
    ] {
