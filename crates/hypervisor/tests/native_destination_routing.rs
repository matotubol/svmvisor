...
fn a_lost_route_lease_holder_bounds_the_incomplete_ipi_wait() {
    ...
    let mut source = NativeIcr::admit(0, &[0, 1]).unwrap();
    let routes = try_lock_routes(&mailboxes).unwrap();
    assert_eq!(source.route_x2avic_startup(0x0000_0001_0000_0500, &mailboxes, |_| panic!("kick")),
        Err(Error::RoutingBusy));
    assert_eq!(source.route_failure().map(|failure| failure.predicate), Some(NativeRoutePredicate::RouteBusy));
