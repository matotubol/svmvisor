...
fn gdt_bytes_and_expanded_segments_match_architectural_layout() {
    ...
    assert_eq!(
        (image.cs().selector, image.cs().attributes, image.cs().limit, image.cs().base),
        (8, 0xa9b, u32::MAX, 0)
    );
    assert_eq!(
        (image.data().selector, image.data().attributes, image.data().limit),
        (16, 0xc93, u32::MAX)
    );
    assert_eq!(
        (image.tr().selector, image.tr().attributes, image.tr().limit, image.tr().base),
        (24, 0x8b, 103, request().tss_base)
    );
    assert_eq!(
        (image.gdtr().selector, image.gdtr().attributes, image.gdtr().limit, image.gdtr().base),
...
fn rejects_noncanonical_ranges_overflow_overlap_and_stack_pointers() {
    for base in [0x8000_0000_0000, 0xffff_7fff_ffff_ffff, 0x7fff_ffff_fff0, u64::MAX - 10] {
        assert_eq!(
            GuestDescriptorRequest { gdt_base: base, ..request() }.validate(),
            Err(DescriptorError::InvalidGdtRange)
        );
        assert_eq!(
            GuestDescriptorRequest { tss_base: base, ..request() }.validate(),
            Err(DescriptorError::InvalidTssRange)
        );
    }
    for tss_base in [0x2000, 0x1f99, 0x2027] {
        assert_eq!(
            GuestDescriptorRequest { tss_base, ..request() }.validate(),
            Err(DescriptorError::OverlappingTables)
        );
    }
    assert_eq!(
        GuestDescriptorRequest { rsp0: 0x8000_0000_0000, ..request() }.validate(),
        Err(DescriptorError::NonCanonicalRsp0)
    );
    assert_eq!(
        GuestDescriptorRequest { ist1: 0x8000_0000_0000, ..request() }.validate(),
        Err(DescriptorError::NonCanonicalIst1)
    );
    for tss_base in [0x1f98, 0x2028, 0x7fff_ffff_ff98, u64::MAX - 103] {
        assert!(GuestDescriptorRequest { tss_base, ..request() }.validate().is_ok());
    }
}
