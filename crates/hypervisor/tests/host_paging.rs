...
fn huge_pages_allow_pat_but_reject_misaligned_address_bits() {
    ...
    assert_eq!(
        walk([0x2007, 0x40003087, 0, 0]),
        Err(WalkError::ReservedEntry { level: 3 })
    );
    assert_eq!(
        walk([0x2007, 0x3007, 0x203087, 0]),
        Err(WalkError::ReservedEntry { level: 2 })
    );
    assert_eq!(
        walk([0x2087, 0, 0, 0]),
...
fn mode_cr3_and_canonical_checks_precede_reads() {
    for (c, va, error) in [
        (
            ...
            0,
            WalkError::FiveLevelUnsupported,
        ),
        ...
        (
            ...
            0,
            WalkError::UnsupportedPhysicalWidth,
        ),
        (
            PagingConfig {
                cr3: 0x1001,
...
fn nxe_and_one_gib_support_are_required_when_used() {
    ...
    assert_eq!(
        translate(
            ...
            0,
            |pa| Some(if pa == 0x1000 { 0x2007 } else { 0x87 })
        ),
...
fn host_admission_still_refuses_every_upper_software_bit_at_every_level() {
    for index in 0..4 {
        for bit in 52..=62 {
            ...
            assert_eq!(
                walk(entries),
                Err(WalkError::UnsupportedEntryBits {
                    level: (4 - index) as u8
                })
            );
        }
