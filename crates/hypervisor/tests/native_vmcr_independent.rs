...
fn legacy(mode32: bool) -> (Vmcb, GuestRegisters) {
    let mut v = Vmcb::new();
    for (offset, value) in [(0x60,1<<24), (0x70,0x7c), (0x410,
        ((if mode32 {0x49b} else {0x9b}) << 16) | (0xffff << 32)),
        (0x4d0,0x1000), (0x558,if mode32 {0x11} else {0x10}),
        (0x570,2), (0x578,0x100)] { put(&mut v,offset,value); }
    (v,GuestRegisters {rcx:0xffff_ffff_c001_0114,rdx:u64::MAX,rbx:0xabc,..Default::default()})
}
...
fn unavailable_svm_contract_survives_the_init_execution_interval() {
    ...
    for mode32 in [false,true] {
        ...
        let mut e=NativeEfer::admit_native(0x500,0,(1<<11)|(1<<20)|(1<<29),0,None).unwrap();
        e.enable_guest_startup(); e.reset_after_init().unwrap();
        f.rcx=0xc0000080;f.rdx=0;put(&mut v,0x78,1);put(&mut v,0x5f8,1<<12);
        let before=(v.guest_rip(),v.guest_rax(),f,e);
        assert_eq!(handle_native_efer(&mut e,&mut v,&mut f,&[0x0f,0x30]),Ok(NativeMsrOutcome::GeneralProtectionPrepared));
...
fn every_low_byte_combination_preserves_fault_and_refusal_priority() {
    for value in 0u64..=255 {
        let (mut v,mut f)=legacy(false);put(&mut v,0x78,1);put(&mut v,0x5f8,value);f.rdx=0;
        let before=(*v.bytes(),f);
        let result=handle_native_vmcr(&mut v,&mut f,&[0x0f,0x30],true);
        match value {
            0|8|16|24 => { assert_eq!(result,Ok(NativeMsrOutcome::Completed));assert_eq!(v.guest_rip(),0x102); }
            0..=31 => { assert_eq!(result,Err(NativeEferError::UnsupportedValue{value}));assert_eq!((*v.bytes(),f),before); }
            _ => { assert_eq!(result,Ok(NativeMsrOutcome::GeneralProtectionPrepared));assert_eq!(v.event_injection(),0x80000b0d);assert_eq!(v.guest_rip(),0x100); }
        }
        assert_eq!(f,before.1);assert_eq!(v.guest_rax(),value);
    }
}
...
fn legacy_ownership_segment_and_virtual_interrupt_limits_remain_transactional() {
    for case in 0..5 {
        ...
        match case { 0=>{},1=>put(&mut v,0x578,0xfffe),2=>put(&mut v,0x558,0x80000011),
            3=>put(&mut v,0x60,(1<<24)|(1<<8)),_=>put(&mut v,0xa8,1<<31) }
        let before=(*v.bytes(),f);
        assert!(handle_native_vmcr(&mut v,&mut f,&[0x0f,0x32],case!=0).is_err(),"case {case}");
