...
fn guest_ram_reuses_retained_coverage_but_never_monitor_or_mmio() {
    ...
    let policy =
        AddressPolicy::new(40, EncryptionState::Unencrypted { encryption_bit: None }).unwrap();
    let monitor = policy.validate(0x400000, 0x100000, 4096).unwrap();
    let descriptors = [
        ram(0x200000, 256),
        MemoryDescriptor { memory_type: 7, ..ram(0x300000, 256) },
        ram(0x400000, 512),
        MemoryDescriptor { memory_type: 11, ..ram(0x600000, 1) },
    ];
    let map = ValidatedMemoryMap::new(&descriptors, 40).unwrap();
    assert!(map.permit_guest_ram(0x2fffff, 2, monitor).is_ok());
    assert_eq!(map.permit_table_entry(0x300000), Err(MemoryError::UntrustedMemoryType));
    assert!(map.permit_guest_ram(0x3ff000, 4096, monitor).is_ok());
    assert_eq!(map.permit_guest_ram(0x3fffff, 2, monitor), Err(MemoryError::MonitorOverlap));
    assert_eq!(map.permit_guest_ram(0x400000, 4096, monitor), Err(MemoryError::MonitorOverlap));
    assert!(map.permit_guest_ram(0x500000, 4096, monitor).is_ok());
    assert_eq!(map.permit_guest_ram(0x600000, 1, monitor), Err(MemoryError::UntrustedMemoryType));
...
fn rejects_malformed_maps_before_any_access() {
    for (descriptors, error) in [
        ...
        (vec![ram(0x1000, 2), ram(0x2000, 1)], MemoryError::UnsortedOrOverlapping),
        (vec![ram(0x2000, 1), ram(0x1000, 1)], MemoryError::UnsortedOrOverlapping),
    ] {
...
fn denies_untrusted_types_and_non_writeback_or_protected_metadata() {
    for ty in [0, 7, 8, 9, 10, 11, 12, 13, 14, 15, u32::MAX] {
        let descriptors = [MemoryDescriptor { memory_type: ty, ..ram(0x1000, 1) }];
    ...
    for attr in [0, 1, 2, 4, 16, 7, 23] {
        let descriptors = [MemoryDescriptor { attributes: attr, ..ram(0x1000, 1) }];
        let map = ValidatedMemoryMap::new(&descriptors, 48).unwrap();
        assert_eq!(map.permit_table_entry(0x1000), Err(MemoryError::MissingWriteBackCapability));
    }
    let descriptors = [MemoryDescriptor { attributes: 8 | 0x2000, ..ram(0x1000, 1) }];
    assert_eq!(
        ValidatedMemoryMap::new(&descriptors, 48).unwrap().permit_table_entry(0x1000),
...
fn ranges_cover_adjacent_descriptors_but_never_gaps_or_untrusted_tail() {
    ...
    assert_eq!(map.permit_table_entry(0x2ff8).unwrap().physical_start(), 0x2ff8);
    assert_eq!(map.permit_table_entry(0x3000), Err(MemoryError::UncoveredRange));
    assert_eq!(map.permit_table_entry(0x1fff), Err(MemoryError::MisalignedEntry));
    let descriptors = [ram(0x1000, 1), ram(0x3000, 1)];
    let map = ValidatedMemoryMap::new(&descriptors, 48).unwrap();
    assert_eq!(map.permit_gdt_copy(0x1ff8, 16), Err(MemoryError::UncoveredRange));
    let descriptors = [ram(0x1000, 1), MemoryDescriptor { memory_type: 11, ..ram(0x2000, 1) }];
    let map = ValidatedMemoryMap::new(&descriptors, 48).unwrap();
    assert_eq!(
...
fn bounded_callbacks_are_invoked_only_after_full_metadata_validation() {
    ...
    assert_eq!(map.read_table_entry(0x1000, |_| None), Err(MemoryError::ReadFailed));
    let mut bytes = [0; 16];
    ...
    assert_eq!(map.permit_gdt_copy(0x1000, 65537), Err(MemoryError::CopyTooLarge));
...
fn writeback_capability_can_coexist_with_other_supported_cache_modes() {
    for attrs in [8, 9, 10, 12, 15, 24, 31] {
        let descriptors = [MemoryDescriptor { attributes: attrs, ..ram(0x1000, 1) }];
        assert!(
            ValidatedMemoryMap::new(&descriptors, 48).unwrap().permit_table_entry(0x1000).is_ok()
        );
    }
}
