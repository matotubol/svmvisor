...
fn write(vmcb: &mut Vmcb, offset: usize, bytes: &[u8]) {
    ...
    unsafe { core::ptr::copy_nonoverlapping(bytes.as_ptr(), (vmcb as *mut Vmcb).cast::<u8>().add(offset), bytes.len()); }
}
#[test]
...
fn zero_or_advanced_filters_and_wrong_exit_cannot_retry() {
    for (offset, bytes) in [(0x03e,vec![0,0]),(0x03c,vec![1,0]),(0x070,vec![0x78]),(0x00e,vec![0])] {
        let mut v=Vmcb::new();
