...
fn hardware_cpuid_completes_without_rewalking_legal_wb_cache_selection() {
    ...
    assert_eq!(reads,1);
    let original=frame;
    assert_eq!(handle_native_cpuid_with_nrip(&mut vmcb,&mut frame,&capabilities(true),[10,11,12,13],true,false,None),
...
fn malformed_or_unowned_hardware_continuations_leave_all_state_unchanged() {
    for case in 0..15 {
        ...
        assert!(handle_native_cpuid_with_nrip(&mut vmcb,&mut frame,&capabilities(supported),[10,11,12,13],startup,false,None).is_err(),"case {case}");
...
fn hardware_continuation_reuses_native_cpuid_policy_and_interrupt_shadow_retirement() {
    ...
    handle_native_cpuid_with_nrip(&mut vmcb,&mut frame,&capabilities(true),[1,2,1<<26,4],true,false,None).unwrap();
...
fn code_mode(vmcb: &mut Vmcb, long_mode: bool, code64: bool, default32: bool, cpl: u64) {
    let attributes = 0x9b | (cpl << 5) | if code64 { 0x200 } else { 0 }
        | if default32 { 0x400 } else { 0 };
    ...
    for (long_mode, code64, default32) in [
        ...
        for cpl in 0..=3 {
            for length in 2..=15 {
                ...
                assert_eq!(handle_native_cpuid_with_nrip(&mut vmcb, &mut frame,
                    &capabilities(true), [10,11,12,13], true, false, Some(&bytes[..length as usize])),
                    Ok(DispatchOutcome::ResumePrepared), "mode {long_mode}/{code64}/{default32} CPL{cpl} len{length}");
...
fn user_cpuid_disable_queues_gp_without_completing_instruction() {
    for cpl in 0..=3 {
        for compat in [false, true] {
            ...
            let original = frame;
            let result = handle_native_cpuid_with_nrip(&mut vmcb, &mut frame,
                &capabilities(true), [10,11,12,13], true, true, Some(&[0x26,0x66,0x67,0x0f,0xa2]));
            if cpl == 0 {
...
fn unsupported_mode_boundaries_and_fault_conflicts_preserve_stopped_state() {
    for case in 0..12 {
        ...
        match case {
            ...
            1 => { put(&mut vmcb,0x578,0xffff_fffe); put(&mut vmcb,0xc8,0x1_0000_0000);
                code_mode(&mut vmcb,true,false,true,3); },
            2 => { put(&mut vmcb,0x578,0xffff_fffe); put(&mut vmcb,0xc8,0);
                code_mode(&mut vmcb,true,false,true,3); }, // Wrapped nRIP.
            3 => { put(&mut vmcb,0x578,0x2000); put(&mut vmcb,0xc8,0x2002);
                put(&mut vmcb,0x410,(0x20_00u64<<32)|(0xfb<<16)|3); }, // CS limit.
            4 => code_mode(&mut vmcb,true,true,true,3), // CS.L and D both set.
            5 => put(&mut vmcb,0x570,0x20002), // VM86 unsupported.
            6 => { code_mode(&mut vmcb,false,false,true,3); startup=false; },
            7 => { code_mode(&mut vmcb,false,false,true,3); put(&mut vmcb,0x558,0x80000001); },
        ...
        let before = *vmcb.bytes(); let original = frame;
        assert!(handle_native_cpuid_with_nrip(&mut vmcb, &mut frame,
            &capabilities(true), [10,11,12,13], startup, true, None).is_err(), "case {case}");
...
fn prefixed_cpuid_requires_exact_non_lock_bytes_and_mode_correct_rex() {
    for code64 in [false, true] {
        for bytes in [
            &[0xf0,0x0f,0xa2][..], &[0xf3,0x0f,0xa2], &[0xf2,0x0f,0xa2],
            &[0x66,0x0f,0xa3], &[0x48,0x66,0x0f,0xa2],
        ] {
            let (mut vmcb, mut frame) = stopped(0x2000);
            code_mode(&mut vmcb,true,code64,!code64,3);
            put(&mut vmcb,0xc8,0x2000 + bytes.len() as u64);
            let before = *vmcb.bytes(); let original = frame;
            assert!(handle_native_cpuid_with_nrip(&mut vmcb,&mut frame,&capabilities(true),
                [10,11,12,13],true,false,Some(bytes)).is_err());
            assert_eq!(vmcb.bytes(),&before); assert_eq!(frame,original);
        }
        let (mut vmcb, mut frame) = stopped(0x2000);
        code_mode(&mut vmcb,true,code64,!code64,3);
        ...
        for bytes in [None,Some(&[0x0f,0xa2][..]),Some(&[0x66,0x66,0x0f,0xa2][..])] {
            assert!(handle_native_cpuid_with_nrip(&mut vmcb,&mut frame,&capabilities(true),
                [10,11,12,13],true,false,bytes).is_err());
            assert_eq!(vmcb.bytes(),&before); assert_eq!(frame,original);
        }
        let result = handle_native_cpuid_with_nrip(&mut vmcb,&mut frame,&capabilities(true),
            [10,11,12,13],true,false,Some(&[0x48,0x0f,0xa2]));
        if code64 { assert_eq!(result,Ok(DispatchOutcome::ResumePrepared)); }
        else { assert!(result.is_err()); assert_eq!(vmcb.bytes(),&before); assert_eq!(frame,original); }
    }
}
...
fn cpuid_prefix_fetch_crosses_pages_in_compatibility_mode_without_changing_other_fetch_modes() {
    ...
    let source = |address: u64, width: usize| match (address,width) {
        (0x1000,8)=>Some(0x2007), (0x2000,8)=>Some(0x3007), (0x3000,8)=>Some(0x4007),
        (0x4008,8)=>Some(0x9007), (0x4010,8)=>Some(0xb007),
        (0x9ffe,1)=>Some(0x66), (0x9fff,1)=>Some(0x67),
        (0xb000,1)=>Some(0x0f), (0xb001,1)=>Some(0xa2), _=>None,
    };
    let before = *vmcb.bytes(); let mut byte_reads=0;
    let bytes = fetch::cpuid_instruction(&vmcb,48,6,4,true,|address,width| {
        byte_reads += usize::from(width==1); source(address,width)
    }).unwrap();
    assert_eq!(&bytes[..4], &[0x66,0x67,0x0f,0xa2]);
    assert_eq!(byte_reads,4); assert_eq!(vmcb.bytes(),&before);
    ...
    for (address,replacement,expected) in [
        ...
        assert_eq!(fetch::cpuid_instruction(&vmcb,48,6,4,true,|a,w| {
...
fn cpuid_prefix_fetch_bounds_entire_span_before_touching_legacy_memory() {
    ...
    put(&mut vmcb,0xc8,0x103); put(&mut vmcb,0x418,0x7000);
    let mut reads=0;
    let result=fetch::cpuid_instruction(&vmcb,48,6,3,true,|address,width| {
        assert_eq!(width,1); assert!((0x7100..0x7103).contains(&address)); reads+=1;
        Some([0x66,0x0f,0xa2][(address-0x7100) as usize])
    }).unwrap();
    assert_eq!(&result[..3],&[0x66,0x0f,0xa2]); assert_eq!(reads,3);
    for length in [0,2,4,16,usize::MAX] {
        assert!(fetch::cpuid_instruction(&vmcb,48,6,length,true,|_,_| panic!("invalid span read")).is_err());
    }
    assert!(fetch::cpuid_instruction(&vmcb,48,6,3,false,|_,_| panic!("unowned read")).is_err());
    put(&mut vmcb,0x418,u32::MAX as u64);
    assert!(fetch::cpuid_instruction(&vmcb,48,6,3,true,|_,_| panic!("wrapped read")).is_err());
}
